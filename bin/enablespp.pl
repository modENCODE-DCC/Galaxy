...
sub Copyinstaller
{
	system ("sudo cp bin/deploy_spp.R /mnt/galaxyData/tmp");
...
foreach my $i (@iplists)
{
	...
	if(searchinstalled($i) == 0)
	{
		...
		{
			system ("sudo R CMD BATCH /mnt/galaxyData/tmp/deploy_spp.R");
		}
		else
		{
			system ("qrsh -l h=${i} -b y \"sudo rm -rf /usr/local/lib/R/site-library/00LOCK ; sudo R CMD BATCH /mnt/galaxyData/tmp/deploy_spp.R\"");
		}
		push (@installed, "$i\n");
